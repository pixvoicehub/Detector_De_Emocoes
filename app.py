...
def get_all_tags_string():
    ...
    # Adiciona a tag customizada de respiração à lista de tags disponíveis
...
def humanize_text_endpoint():
    ...
    # [NOVA ALTERAÇÃO] Captura o novo parâmetro 'add_smiling_tone' do frontend.
    ...
    try:
        ...
        # [NOVA ALTERAÇÃO] Adiciona a instrução para o tom sorridente se o checkbox estiver marcado.
        if add_smiling_tone:
...
if __name__ == '__main__':
    # [ALTERAÇÃO SUGERIDA] Para produção no Render, é melhor usar um servidor WSGI como o gunicorn.
    # O debug=True não deve ser usado em produção. O Render geralmente lida com isso.
    # Se você precisar definir a porta, use: port = int(os.environ.get("PORT", 8080))
